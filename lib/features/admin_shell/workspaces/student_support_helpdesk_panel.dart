...
class StudentSupportHelpdeskPanel extends StatefulWidget {
  ...
  @override
  State<StudentSupportHelpdeskPanel> createState() => _StudentSupportHelpdeskPanelState();
}

class _StudentSupportHelpdeskPanelState extends State<StudentSupportHelpdeskPanel> {
  ...
  static const _tickets = [
    _SupportTicket(
      ...
      summary: 'CSC 309 exam score is missing after rewrite and still appears as carryover.',
      sla: 'Due today',
    ),
    _SupportTicket(
    ...
    _SupportTicket(
      ...
      status: 'Escalated',
      owner: 'Exam Office',
      summary: 'Candidate was disconnected during CBT upload and needs submission verification.',
      sla: 'Overdue',
    ),
    _SupportTicket(
      ...
      status: 'Resolved',
      owner: 'Lecturer',
      summary: 'Assignment file uploaded late due to network issue; lecturer feedback requested.',
      sla: 'Closed',
    ),
  ];
  ...
  static const _activity = [
    _SupportActivity(
      ...
      detail: 'CSC 309 result batch and student academic record are being reconciled.',
      time: 'Today, 12:08',
      severity: 'High',
    ),
    _SupportActivity(
  ...
  Widget build(BuildContext context) {
    ...
    final filtered = _tickets
        .where((ticket) => _selectedCategory == 'All' || ticket.category == _selectedCategory)
        .where((ticket) => _selectedStatus == 'All' || ticket.status == _selectedStatus)
        .toList();
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Wrap(spacing: 10, runSpacing: 10, children: const [
            _HelpdeskChip(label: 'Open tickets: 38', icon: Icons.mark_unread_chat_alt_outlined),
            _HelpdeskChip(label: 'Missing results: 9', icon: Icons.workspace_premium_outlined),
            _HelpdeskChip(label: 'Registration issues: 11', icon: Icons.app_registration_outlined),
            _HelpdeskChip(label: 'Exam follow-up: 6', icon: Icons.assignment_outlined),
            _HelpdeskChip(label: 'Overdue SLA: 4', icon: Icons.timer_off_outlined),
          ]),
            ...
            SizedBox(
              ...
              child: DropdownButtonFormField<String>(
                ...
                items: const [
                  ...
                  DropdownMenuItem(value: 'Exam Incident', child: Text('Exam Incident')),
          ...
          const SizedBox(height: 10),
          for (final activity in _activity) _SupportActivityTile(activity: activity),
        ]),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: ticket.priority == 'High' ? scheme.error.withValues(alpha: 0.4) : scheme.outlineVariant),
      ),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Wrap(spacing: 12, runSpacing: 8, alignment: WrapAlignment.spaceBetween, children: [
          ConstrainedBox(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text('${ticket.ticketNo} • ${ticket.category}', style: const TextStyle(fontWeight: FontWeight.w900)),
              const SizedBox(height: 4),
              Text('${ticket.student} • ${ticket.matricNo}', style: TextStyle(color: scheme.onSurfaceVariant)),
            ]),
          ),
          _StatusBadge(text: ticket.priority, color: priorityColor),
        ]),
        const SizedBox(height: 10),
        Text(ticket.summary, style: TextStyle(color: scheme.onSurfaceVariant)),
        const SizedBox(height: 10),
...
class _SupportActivityTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final scheme = Theme.of(context).colorScheme;
    final severityColor = activity.severity == 'High' ? scheme.error : scheme.primary;
    return ListTile(
      ...
      title: Text('${activity.title} • ${activity.ticketNo}', style: const TextStyle(fontWeight: FontWeight.w800)),
      subtitle: Text('${activity.actor} • ${activity.detail} • ${activity.time}'),
