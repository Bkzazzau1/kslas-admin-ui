...
class _ExamManagementPanelState extends State<ExamManagementPanel> {
  ...
  Widget build(BuildContext context) {
    ...
    final filtered = _exams
        .where((exam) => _selectedStatus == 'All' || exam.readiness == _selectedStatus)
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Wrap(spacing: 10, runSpacing: 10, children: const [
            _ExamChip(label: 'Scheduled exams: 42', icon: Icons.event_available_outlined),
            _ExamChip(label: 'Question approval: 9', icon: Icons.rule_folder_outlined),
            _ExamChip(label: 'Invigilator gaps: 4', icon: Icons.person_add_alt_1_outlined),
            _ExamChip(label: 'Open incidents: 3', icon: Icons.report_problem_outlined),
          ]),
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: exam.readiness == 'Needs Attention' ? scheme.error.withValues(alpha: 0.4) : scheme.outlineVariant,
        ),
      ),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Wrap(spacing: 12, runSpacing: 8, alignment: WrapAlignment.spaceBetween, children: [
          ConstrainedBox(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text('${exam.courseCode} • ${exam.title}', style: const TextStyle(fontWeight: FontWeight.w900)),
              const SizedBox(height: 4),
              Text('${exam.mode} • ${exam.dateTime} • ${exam.venue}', style: TextStyle(color: scheme.onSurfaceVariant)),
            ]),
          ),
...
class _IncidentTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ListTile(
      ...
      title: Text('${incident.exam} • ${incident.report}', style: const TextStyle(fontWeight: FontWeight.w800)),
