...
class _AcademicStructurePanelState extends State<AcademicStructurePanel> {
  ...
  Widget build(BuildContext context) {
    ...
    final filteredProgrammes = _programmes
        .where((item) => _selectedFaculty == 'All' || item.faculty == _selectedFaculty)
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Wrap(spacing: 10, runSpacing: 10, children: const [
            _StructureChip(label: 'Faculties: 8', icon: Icons.account_balance_outlined),
            _StructureChip(label: 'Departments: 42', icon: Icons.account_tree_outlined),
            _StructureChip(label: 'Programmes: 96', icon: Icons.school_outlined),
            _StructureChip(label: 'Curriculum maps: 1,248', icon: Icons.menu_book_outlined),
            _StructureChip(label: 'Active cohorts: 62', icon: Icons.groups_2_outlined),
          ]),
          ...
          Wrap(spacing: 10, runSpacing: 10, children: [
            SizedBox(
              ...
              child: DropdownButtonFormField<String>(
                ...
                onChanged: (value) => setState(() => _selectedFaculty = value ?? 'All'),
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: item.status == 'Review' ? scheme.error.withValues(alpha: 0.4) : scheme.outlineVariant),
      ),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Wrap(spacing: 12, runSpacing: 8, alignment: WrapAlignment.spaceBetween, children: [
          ConstrainedBox(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text('${item.programme} • ${item.code}', style: const TextStyle(fontWeight: FontWeight.w900)),
              const SizedBox(height: 4),
              Text('${item.faculty} • ${item.department}', style: TextStyle(color: scheme.onSurfaceVariant)),
            ]),
          ),
...
class _CurriculumTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final scheme = Theme.of(context).colorScheme;
    final statusColor = item.status == 'Needs Review' ? scheme.error : scheme.primary;
    return ListTile(
      ...
      title: Text('${item.courseCode} • ${item.title}', style: const TextStyle(fontWeight: FontWeight.w800)),
      subtitle: Text('${item.programme} • ${item.level} Level • Semester ${item.semester} • ${item.credits} credits • ${item.type}'),
