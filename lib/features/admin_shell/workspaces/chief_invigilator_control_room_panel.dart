...
class ChiefInvigilatorControlRoomPanel extends StatefulWidget {
  ...
  @override
  State<ChiefInvigilatorControlRoomPanel> createState() => _ChiefInvigilatorControlRoomPanelState();
}

class _ChiefInvigilatorControlRoomPanelState extends State<ChiefInvigilatorControlRoomPanel> {
  ...
  static const _sessions = [
    ...
    _CommandSession(
      ...
      risk: 'High',
      status: 'Live',
      decision: 'Review repeated face-away and device-change alerts before closure.',
    ),
    _CommandSession(
  ...
  static const _decisions = [
    _CommandDecision(
      ...
      reason: 'Multiple high-confidence events across the same candidate session.',
      status: 'Escalated',
      severity: 'High',
      time: 'Today, 10:38',
    ),
    _CommandDecision(
      ...
      reason: 'Two app ID exceptions require exam officer review before closure.',
      status: 'Pending',
      severity: 'Medium',
      time: 'Today, 10:44',
    ),
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Wrap(spacing: 10, runSpacing: 10, children: const [
            _ControlChip(label: 'Live sessions: 15', icon: Icons.play_circle_outline),
            _ControlChip(label: 'Rooms/groups: 42', icon: Icons.meeting_room_outlined),
            _ControlChip(label: 'Open incidents: 24', icon: Icons.report_problem_outlined),
            _ControlChip(label: 'Escalated: 7', icon: Icons.priority_high_outlined),
            _ControlChip(label: 'Closure holds: 4', icon: Icons.lock_clock_outlined),
          ]),
          ...
          for (final session in filtered) _CommandSessionTile(session: session),
          const SizedBox(height: 18),
          Text(
            'Command decisions',
            style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w800),
          ...
          const SizedBox(height: 10),
          for (final decision in _decisions) _CommandDecisionTile(decision: decision),
        ]),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 10),
        Text(session.decision, style: TextStyle(color: scheme.onSurfaceVariant)),
        const SizedBox(height: 12),
...
class _CommandDecisionTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ListTile(
      ...
      title: Text('${decision.title} • ${decision.scope}', style: const TextStyle(fontWeight: FontWeight.w800)),
      subtitle: Text('${decision.owner} • ${decision.reason} • ${decision.time}'),
